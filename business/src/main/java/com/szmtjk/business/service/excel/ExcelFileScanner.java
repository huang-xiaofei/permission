...
/**
 * excel文件自动扫描器，扫描器工作原理： 1.
 * 扫描指定的体检报告zip压缩文件目录，遍历所有zip文件并解压至指定excel文件目录，成功后，删除zip文件 2.
 * 扫描指定excel文件目录，遍历所有excel文件并解析 2.1 对于解压成功后的excel文件直接删除 2.2
 * 对于解压失败后的excel文件移除至指定错误目录
 */
@Service
public class ExcelFileScanner {
	...
	private void loopZipFilesAndUnzip(String zipFileDir, String unzippedDir) {
		try {
			...
			LOG.error("ExcelFileScanner.loopZipFilesAndUnzip err, zipFileDir:{}, unzippedDir:{}", zipFileDir,
					unzippedDir, e);
		}
	}
	...
	public void scanAndDisposeExcel() {
		loopExcelFilesAndDispose(reservationExcelUnzippedDir, reservationInvalidExcelFileDir,
				BizExcelDisposer.EXAM_RESERVATION);
	...
	private void loopExcelFilesAndDispose(String excelDir, String invalidFileDir, String disposeKey) {
		...
		try {
			...
			LOG.error("ExcelFileScanner.loopExcelFilesAndDispose, excelDir:{}, invalidFileDir:{}, disposeKey:{}",
					excelDir, invalidFileDir, disposeKey, e);
		}
	}
