...
@Service
public class ActionResourcesServiceImpl
		extends BaseCRUDService<ActionResourcesDO, ActionResources, ActionResourcesDBQuery, ActionResourcesQuery>
		implements ActionResourcesService {
	...
	@Override
	public List<ActionResources> queryByUserIdAndSystemId(Long userId, int systemId) {
		List<ActionResources> resources = new ArrayList<ActionResources>();
		List<ActionResourcesDO> resourcesDOList = this.actionResourcesDAO.queryByUserIdAndSystemId(userId, systemId);
		if (!CollectionUtils.isEmpty(resourcesDOList)) {
			for (ActionResourcesDO resourcesDO : resourcesDOList) {
				resources.add(this.actionResourcesConverter.toModel(resourcesDO));
			}
