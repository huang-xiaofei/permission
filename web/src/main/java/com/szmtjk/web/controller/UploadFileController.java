...
public class UploadFileController extends BaseController {
	...
	public JsonRet<Object> upLoad(@RequestParam MultipartFile fileName, HttpServletRequest request) throws Exception {
		...
		params.put("filePath", request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
				+ "/xyl/upload/" + newFileName);
	...
	@ResponseBody
	public JsonRet<Object> uploadExamReportZip(@RequestParam MultipartFile zipFile, HttpServletRequest request)
			throws Exception {
	...
	@ResponseBody
	public JsonRet<Object> uploadExamReservationZip(@RequestParam MultipartFile zipFile, HttpServletRequest request)
			throws Exception {
	...
	private boolean createUploadFile(MultipartFile zipFile, String uploadDir) {
		try {
			...
			FileUtils.copyInputStreamToFile(zipFile.getInputStream(),
					new File(uploadDir, zipFile.getOriginalFilename()));
			return true;
		} catch (Exception e) {
			LOG.error("createUploadFile err, uploadDir:{}, newFileName:{}", uploadDir, zipFile.getOriginalFilename(),
					e);
		}
		return false;
	}
