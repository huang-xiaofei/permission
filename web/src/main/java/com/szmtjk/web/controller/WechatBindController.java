...
public class WechatBindController {
	...
	/**
	 * 获取公众号绑定后台账号url地址 公众号菜单固定维护一个请求链接，具体的跳转url由后台指定
	 * 
	 * @return
	 */
	...
	/**
	 * 根据跳转后的页面获取的微信code，来后端校验是否已经绑定了某个后台账号， 如果已经绑定了则返回已绑定的用户名，则在页面中显示绑定信息
	 * 否则跳转至绑定账号的输入页面
	 * 
	 * @param code
	 * @return
	...
	/**
	 * 公众号页面校验是否绑定 如果已经绑定则返回已绑定的手机号（打码），并在页面中显示绑定信息 否则跳转至绑定账号的输入页面
	 * 
	 * @param code
	 * @return
	 */
