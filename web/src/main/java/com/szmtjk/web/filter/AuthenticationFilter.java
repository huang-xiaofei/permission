...
/**
 * @author tsingtao_tung Created At: 2018/1/21 下午11:57.
 */
@Order(1)
...
public class AuthenticationFilter implements Filter {
	...
	private static final List<String> URL_WHITE_LIST = Lists.newArrayList("/test/", "/webjars", "/api",
			"/swagger-ui.html", "/swagger-resources", "/v2/api-docs", "/wechat/", "/mobile/", "/signin/");
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!isRequestStartsWithInWhitelist(requestPath)) {// 非白名单的进行token验证
			...
			if (!isAuthenticated) {
				System.out.println("当前token:" + token + ",校验未通过!!!");
